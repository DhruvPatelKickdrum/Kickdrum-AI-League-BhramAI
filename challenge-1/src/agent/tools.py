...
def route_claim(claim: Annotated[str, "The claim text to classify"]) -> dict:
    ...
    logger.info("Tool: route_claim(%s)", claim[:80])
...
def search_static_kb(query: Annotated[str, "Search query for the knowledge base"]) -> dict:
    ...
    logger.info("Tool: search_static_kb(%s)", query[:80])
...
def scrape_news(query: Annotated[str, "Search query for news articles"]) -> list[dict]:
    ...
    logger.info("Tool: scrape_news(%s) domains=%s", query[:80], get_allowed_domains("news"))
...
def scrape_finance(query: Annotated[str, "Search query for financial data"]) -> list[dict]:
    ...
    logger.info("Tool: scrape_finance(%s) domains=%s", query[:80], get_allowed_domains("finance"))
...
def scrape_government(query: Annotated[str, "Search query for government policies"]) -> list[dict]:
    ...
    logger.info("Tool: scrape_government(%s) domains=%s", query[:80], get_allowed_domains("govt"))
...
def fetch_weather(query: Annotated[str, "Weather query including location"]) -> list[dict]:
    ...
    logger.info("Tool: fetch_weather(%s)", query[:80])
...
def store_fact(
    ...
    domain: Annotated[str, "The domain: news, finance, govt, weather, or null"],
) -> dict:
    """Evaluate if evidence is stable enough to store in the knowledge base, and store if yes."""
    logger.info("Tool: store_fact(domain=%s)", domain)
...
def verify_and_synthesize(
    ...
    if evidence is None:
        evidence = []
    logger.info("Tool: verify_and_synthesize(%s) evidence_count=%d", claim[:80], len(evidence))
